...
!(async () => {
  ...
  for (i = 0; i < app_soy_wx_jysz_token.length; i++) {
    ...
    if (!soy_wx_jysz_User_Agent) {
      ...
      soy_wx_jysz_headers = {
        "Host": "apponlie.sahaj.cn", "Accept": "application/json", "Origin": "http://ppllmm.zhuwentao52.top", "User-Agent": `${soy_wx_jysz_User_Agent}`, "token": `${soy_wx_jysz_token}`,
    ...
    $.get({
      ...
      if (result.code == 0) {
        ...
        if (taskId !== null) {
          ...
          const cTime = Math.floor(Math.random() * (6000) + 10000)
          console.log("阅读中等待...",Math.floor(cTime/1000),"秒");
          await $.wait(cTime)
...
function soy_jysz_task(data) {
  return new Promise((resolve, reject) => {
    $.post({
      ...
      headers: {
        "Accept": "application/json", "Content-Type": "application/json;charset=UTF-8", "Host": "apponlie.sahaj.cn", "Origin": "http://ppllmm.zhuwentao52.top", "Referer": "http://ppllmm.zhuwentao52.top", "token": soy_wx_jysz_token, "User-Agent": soy_wx_jysz_User_Agent, "X-Requested-With": "com.tencent.mm"
      },
      body: `${data}`,
    }, async (error, response, data) => {
      //console.log(data)
      ...
      if (result.code == 0) {
        ...
        const cTime = Math.floor(Math.random() * (49000) + 2000)
        console.log("阅读中等待...",Math.floor(cTime/1000),"秒");
        await $.wait(cTime)
        await soy_jysz_fetchTask()
      } else {
